...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        //スクロールするスプライトの親ノード（スクロールを一括で停止するためのノード）
        scrollNode = SKNode()
        addChild(scrollNode)
    ...
    func setupGround() {
        ...
        //画像が多少荒くなってでも処理速度を高める設定
        groundTexture.filteringMode = .nearest
        
        //必要な地面の枚数を計算（途切れないように少し多め）
        let needNumber = Int(self.frame.size.width / groundTexture.size().width) + 2
        
        //スクロールするアクションを作成
        //左方向に画像1枚分スクロールさせるアクション
    ...
    func setupItem() {
        ...
        // アイテム位置の上下の振れ幅を鳥のサイズの1/2とする
        let item_random_y_range = birdSize.height * 0.5
        
        // 下の壁のY軸下限位置(中央位置から下方向の最大振れ幅で下の壁を表示する位置)を計算
        let groundSize = SKTexture(imageNamed: "ground").size()
        let center_y = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        let item_lowest_y = center_y - itemTexture.size().height / 2 - item_random_y_range / 2
        ...
        let createItemAnimation = SKAction.run({
            ...
            //0〜item_random_y_rangeまでのランダム値を生成
            let item_random_y = CGFloat.random(in: 0..<item_random_y_range)
            //Y軸の下弦にランダムな値を足して、下の壁のY座標を決定
            let item_y = item_lowest_y + item_random_y
            
            //アイテムを作成
            let coin = SKSpriteNode(texture: itemTexture)
            coin.position = CGPoint(x: 0, y: item_y)
            
            // スプライトに物理演算を設定する
            coin.physicsBody = SKPhysicsBody(rectangleOf: itemTexture.size())
        ...
        let itemWaitAnimation = SKAction.wait(forDuration: 7)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            //音を出す
            let soundAction: SKAction = SKAction.playSoundFileNamed("coin.mp3", waitForCompletion: true)
            self.run(soundAction)
            
            //アイテムを一度消して、setupItemメソッドを呼び出す
